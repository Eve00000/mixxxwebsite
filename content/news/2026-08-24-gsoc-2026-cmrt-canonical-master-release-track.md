...
The solution is to use the *complete* Chromaprint fingerprint, the full duration, not just the first two minutes, as an identifier for a specific mastering. Two copies of a track with identical complete fingerprints are provably the same mastering.

Digital rips can have different lengths of lead-in/lead-out. This leads to an offset in the audio of otherwise identical tracks.
![Two tracks of the same mastering with misaligned hotcues due to a pregap offset]({static}/images/news/cmrt_pregap_offset_misalignment.png)
This happens if you try to share a beat grid.

The pregap offset between any two masterings can be calculated, and eventually this information can be shared through MusicBrainz so DJs worldwide can retrieve timing metadata that is guaranteed to match their specific version of a track.
...
This project builds the entire Mixxx-side foundation: database schema, fingerprint storage, background AcoustID lookup, preferences UI, queue view, and track menu integration.

The result: once two tracks share a CMRT, we can trust they're the same mastering — beatgrid, cue points, and hotcues, everything will actually line up.
